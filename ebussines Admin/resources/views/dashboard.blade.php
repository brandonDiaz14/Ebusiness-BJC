                ...
                <center>
                                ...
                                @foreach($carrito as $car)
                                    @if($car->usuario == Auth::user()->id)
                                        <div class="block px-4 py-2 text-xs text-gray-400">
                                            ...
                                            <?php $compra .= $car->nombre_poducto . " (precio:" . ($car->precio_producto * $car->cantidad_productos) . ") (cantidad: " . $car->cantidad_productos . ")\n"; ?>
                                        </div>
                                    @endif
                                @endforeach
                                <center>
                    ...
                    <div name="carrito" style="margin: 5px; text-align: right; width: 10%;">
                        <x-dropdown style>
                            <x-slot name="trigger">
                                <button class="btn btn-light">
                                    <p style="color: black;">Carrito de clientes</p>
                                </button>
                            </x-slot>
                            <x-slot name="content">
                                <!-- Account Management -->
                                <div class="block px-4 py-2 text-xs text-gray-400">
                                    {{ __('Lo que tienen los usuarios en su carrito') }}
                                </div>
                                @foreach($user as $car)
                    ...
                    <div class="card mt-3" style="height: 600px; border-radius: 20px;">
                        ...
                        <div class="card-body" style="border-radius: 20px; position: relative;">
                            <div style="text-align: center;">
                                <h6 style="font-size: 20px;" class="display-6" >{{ $produ->Nom_produ }}</h6>
                                ...
                                <form action="{{route('guardar.imagen')}}" method="POST">
                                    @csrf
                                    <br>
                                    <p>Agregar o Actualizar Imagen</p>
                                    <p>Link de la Imagen</p>
                                    
                                    <input type="txt" class="form-control"  name="nombre_imagen">
                                    <br>
                                    <input type="hidden" value="{{$produ->id_produ}}" name="producto_id_produ">
                                    <button type="submit" class="btn btn-primary" style="color: black;">Agregar Imagen</button>
